...
{
    public static List<FlightData> GenerateMockFlights(string provider, string origin, string destination, DateTime departureDate, DateTime returnDate, int passengerCount)
    {
        var faker = new Faker();
        ...
        for (int i = 0; i < 5; i++)
        {
            flights.Add(new FlightData
            {
                ...
                ArrivalTime = returnDate.AddHours(faker.Random.Int(3, 15)).ToString("yyyy-MM-ddTHH:mm:ss"),
